...
def on_click(event):
    ...
    annotation = plt.annotate(settings.NUM_TRACK_POINTS, [event.xdata, event.ydata])
...
def on_t_press(event):
    ...
    # Generate track
    (
        settings.CENTER_LINE,
        right_line,
        left_line,
        settings.YELLOW_CONES,
        settings.BLUE_CONES,
        settings.ORANGE_CONES,
    ) = tg.create_track(np.array(settings.CLICKED_POINTS))

    # Plot centerline and border lines
    settings.CENTER_LINE_PLOT = plt.plot(
        settings.CENTER_LINE[:, 0], settings.CENTER_LINE[:, 1], "-k"
    )[0]
    plt.plot(right_line[:, 0], right_line[:, 1], "--y", alpha=0.5)
    plt.plot(left_line[:, 0], left_line[:, 1], "--b", alpha=0.5)

    # Plot the cones
    plt.plot(settings.BLUE_CONES[:, 0], settings.BLUE_CONES[:, 1], "*b")
    plt.plot(settings.YELLOW_CONES[:, 0], settings.YELLOW_CONES[:, 1], "*y")
    plt.plot(
        settings.ORANGE_CONES[:, 0],
        settings.ORANGE_CONES[:, 1],
        linestyle="None",
        marker="*",
        color="tab:orange",
    )
    ...
    # Update status
    plt.title(
        "Track generated. Press 'h' to toggle centerline and 'd' to save to .csv file"
    )
    settings.FIG.canvas.draw()
...
def on_h_press(event):
    ...
    for track_point, annotation in zip(
        settings.TRACK_POINT_PLOTS, settings.ANNOTATIONS
    ):
...
def on_d_press(event):
    ...
    blue_text = np.full(fill_value="b", shape=(len(settings.BLUE_CONES), 1), dtype=str)
    yellow_text = np.full(
        fill_value="y", shape=(len(settings.YELLOW_CONES), 1), dtype=str
    )
    orange_text = np.full(
        fill_value="o", shape=(len(settings.ORANGE_CONES), 1), dtype=str
    )
    center_text = np.full(
        fill_value="c", shape=(len(settings.CENTER_LINE), 1), dtype=str
    )
    ...
    center_with_field = np.hstack([settings.CENTER_LINE, center_text])
    ...
    while os.path.exists(
        "./generated_files/track_{0:03d}.csv".format(current_track_index)
    ):
    ...
    with open(f"./generated_files/{track_file_name}", "w", newline="") as file:
        ...
        writer.writerows(center_with_field)
        file.close()
